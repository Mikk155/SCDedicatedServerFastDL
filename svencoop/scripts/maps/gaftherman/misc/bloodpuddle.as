...
void EntityDied()
{
	...
	while((@pDeadEntity = g_EntityFuncs.FindEntityByClassname( pDeadEntity, "*" )) !is null )
	{
		...
		{
			g_Game.AlertMessage( at_console, string(pDeadEntity.pev.classname)+'\n');

			CBaseEntity@ pBlood = g_EntityFuncs.CreateEntity( "blood", null, true);
			pBlood.pev.origin = pDeadEntity.pev.origin;
...
class blood : ScriptBaseAnimating
{
	...
	void BloodThink()
	{
		...
		{
			g_Game.AlertMessage( at_console, string(pOwner.pev.classname)+'\n');
