...
void idletestfunc(){ 
	...
	if(m_sMap == "hcl_lobby") return;
	if(m_sMap == "tln_m1") return;
	if(m_sMap == "of_c00") return;
	if(m_sMap == "of_c07") return;
	
  int plyCnt = 0;
  int g_maxplayersAFK = 60;
	...
	for( int i = 1; i <= g_Engine.maxClients; ++i )	{
      ...
      if(
        ...
        if(totalVal > g_maxTotalKick){
          g_PlayerFuncs.ClientPrintAll( HUD_PRINTTALK, "Player '" + pPlayer.pev.netname + "' was slayed for being AFK.\n" ); 
          g_EngineFuncs.ServerCommand("kick \"#" + g_EngineFuncs.GetPlayerUserId(pPlayer.edict()) + "\" \"You were slayed for being AFK.\"\n");
